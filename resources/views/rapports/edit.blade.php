        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    Modifier le rapport de stage
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route('rapports.update', $rapport->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            ...
                            <select name="stage_id" id="stage_id" class="form-select @error('stage_id') is-invalid @enderror" required>
                                @foreach($stages as $stage)
                                    <option value="{{ $stage->id }}" 
                                        {{ old('stage_id', $rapport->stage_id) == $stage->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            <label for="fichier" class="form-label">Nouveau fichier du rapport (laisser vide pour ne pas modifier)</label>
                            <input type="file" class="form-control @error('fichier') is-invalid @enderror" 
                                   id="fichier" name="fichier">
                            <div class="form-text">
                                Fichier actuel : 
                                <a href="{{ route('rapports.telecharger', $rapport->id) }}" target="_blank">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('commentaire_etudiant') is-invalid @enderror" 
                                      ...
                                      rows="3">{{ old('commentaire_etudiant', $rapport->commentaire_etudiant) }}</textarea>
